...
class CANWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # Set CAN up button
        canup_btn = QPushButton(f"Set {can_line} up")
        canup_btn.clicked.connect(self.set_canup)
        canup_loopback_btn = QPushButton(f"Set {can_line} up loopback on")
        canup_loopback_btn.clicked.connect(self.set_canup_loopback_on)
        canup_layout = QHBoxLayout()
        canup_layout.addWidget(canup_btn)
        canup_layout.addWidget(canup_loopback_btn)
        ...
        left_layout.addLayout(canup_layout)
    ...
    def set_canup(self):
        self.cansend_queue.put(f"sudo -S ip link set {can_line} down")
        self.cansend_queue.put(f"sudo ip link set {can_line} up type can bitrate 500000 dbitrate 1000000 fd on")

    def set_canup_loopback_on(self):
        self.cansend_queue.put(f"sudo -S ip link set {can_line} down")
        self.cansend_queue.put(f"sudo ip link set {can_line} up type can bitrate 500000 dbitrate 1000000 fd on loopback on")
