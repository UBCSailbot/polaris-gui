    ...
    try:
        if colour is not None:
            pen = pg.mkPen(
                colour,
                width=line_width,
                style=QtCore.Qt.DashLine if line_dashed else None,
            )
...
def create_heading_arrow(angle, brush) -> HeadingArrow:
    ...
    return HeadingArrow(
        angle=(angle + 90) % 360,
        headLen=cg.h_arrow_headLen,
        tailLen=cg.h_arrow_tailLen,
        headWidth=cg.h_arrow_headWidth,
        tailWidth=cg.h_arrow_tailWidth,
        pen=cg.h_arrow_pen,
        brush=brush,
    )
...
class GraphObject:  # struct which keeps together objects needed for a graph
    ...
    def initialize(self):
        self.graph = create_graph(
            ...
            else f"{self.x_name} vs. {self.y_name}",
...
class DataObject:
    def __init__(
        ...
        self.line_dashed = (
            line_dashed  # boolean indicating whether line should be dashed or not
        )
    ...
    def init_empty_line(self) -> None:
        self.line = generic_create_line(
            ...
            self.line_dashed,
            self.symbol_brush,
            symbol="o" if self.symbol_brush else None,
        )  # should automatically create line w/ empty data
    ...
    def get_current(self):
        ...
        return (
            self.current,
            val,
        )  # returns the time, value of most recently logged datapoint
    ...
    def remove_datapoint(self, x):
        try:
            ...
            print(
                "ERR - trying to apply remove_datapoint() on a point which does not exist"
            )
    ...
    def update_data(self, current_time, scroll_window):
        ...
        for key in keys:
            if key < (
                current_time - scroll_window - 5
            ):  # if value is outside graph plus some margin of time
                del self.data[key]
        self.update_line_data()
        return
    ...
    def update_label(self):
        if self.label is not None:
            self.label.setText(f"{self.name}: {self.get_current()[1]} {self.units}")
        return
...
class PIDObject(DataObject):
    ...
    def parse_frame(self, current_time, data_line, parsed_dict=None):
        ...
        if parsed_dict is None:
            ...
            if (
                ...
                desired_arrow = create_heading_arrow(
                    parsed_dict[cg.desired_heading_arrow_name], cg.h_arrow_desired_brush
                )
            if (
    ...
    def get_current(self):
        ...
        return (
            self.current,
            val,
        )  # returns the time, value (full dict) of most recently logged datapoint
    ...
    def update_data(self, current_time, scroll_window):
        ...
        for time_logged in self.data.keys():
            if (
                (current_time - time_logged) > self.timeout_duration
    ...
    def remove_datapoint(self, x):
        try:
            if self.data[x][cg.desired_heading_arrow_name] is not None:
                self.graph_obj.graph.removeItem(
                    self.data[x][cg.desired_heading_arrow_name]
                )
            if self.data[x][cg.actual_heading_arrow_name] is not None:
                self.graph_obj.graph.removeItem(
                    self.data[x][cg.actual_heading_arrow_name]
                )
...
class AISObject(DataObject):
    ...
    def initialize(self, timestamp=None):
        ...
        self.polaris_line = generic_create_line(
            self.graph_obj,
            "POLARIS",
            [],
            [],
    ...
    def add_frame(self, x, y, key, data, x_key):
        ...
        if key in self.dataset:  # if ship already exists in dataset
            self.remove_datapoint(
                self.dataset[key][x_key]
            )  # remove old lon/lat value from graph

        self.dataset[key] = (
            data  # NOTE: instead of appending, replace on ship SID - should self.dataset be a dict instead? Check that everything works like this
        )
        ...
        self.add_datapoint(
            x, y
        )  # add new (lon, lat) of ship to list of datapoints to graph
    ...
    def update_data(self, current_time, scroll_window):
        ...
        for key, frame in self.dataset.items():
            if (
                current_time - frame[cg.LAST_UPDATED] > cg.data_timeout
            ):  # if data has not been updated for long enough: remove dp
...
class Docker_Commands(Enum):
    START = """ros2 launch global_launch main_launch.py record:=true mode:=production \
            log_level:=debug config:=on_water_globals.yaml 2>&1 | tee src/global_launch/voyage_log/combined_log_$( date +%F_%T).txt"""
    STOP = ""
