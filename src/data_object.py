...
def create_label(
    title, min_width=cg.value_label_min_width, max_height=cg.value_label_max_height
):
    label = QLabel(title)
...
def create_line(
    graph_obj, name, x_data, y_data, colour, line_width, line_dashed, symbol=None
):
    """
    ...
    try:
        pen = pg.mkPen(
            colour, width=line_width, style=QtCore.Qt.DashLine if line_dashed else None
        )

        new_line = graph_obj.graph.plot(
            x_data, y_data, name=name, pen=pen, symbol="o" if symbol else None
        )
        return new_line
...
class GraphObject:  # struct which keeps together objects needed for a graph
    def __init__(
        self, x_name, y_name, x_units, y_units, minn, maxn, dropdown_label=None
    ):  # data = history?
        """
...
class DataObject:
    def __init__(
        ...
        if line_colour is None and graph is not None:
            raise ValueError(
                "DataObject __init__: Given a graph, but not a line colour"
            )
    ...
    def get_current(self):
        val = (
            round(self.data.get(self.current), self.dp)
...
class AISObject(
    DataObject
):  # NOTE: does this class need to take all arguments of parent class?
    def __init__(
        self,
        name,
        dp,
        ...
        self.datasets = [
            [],
            [],
        ]  # contains data for previous cycle and current cycle - each batch of AIS messages is separated
