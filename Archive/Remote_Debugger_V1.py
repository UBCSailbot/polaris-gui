...
class CANWindow(QWidget):
    ...
    def init_ui(self):
        ...
        # === Live Value Display ===
        self.live_values_label = QLabel("Latest Values: --")
        self.live_values_label.setAlignment(Qt.AlignLeft)
        self.live_values_label.setStyleSheet("""
            color: white;
            font-size: 14px;
            font-weight: bold;
            padding: 4px;
        ...
        # === Plot Widgets ===
        self.temp_plot = pg.PlotWidget(title="Temperatures vs Time")
        self.temp_plot.setYRange(0, 100)
        self.temp_plot.setMouseEnabled(x=True, y=True)
        self.temp_plot.addLegend()
        self.temp1_curve = self.temp_plot.plot(pen='r', name='Temp 1')
        self.temp2_curve = self.temp_plot.plot(pen='g', name='Temp 2')
        self.temp3_curve = self.temp_plot.plot(pen='y', name='Temp 3')
        self.temp_plot.setLabel('left', 'Temp (°C)')
        self.temp_plot.setLabel('bottom', 'Time (s)')

        self.volt_plot = pg.PlotWidget(title="Cell Voltages vs Time")
        self.volt_plot.setYRange(0, 5)
        self.volt_plot.setMouseEnabled(x=True, y=True)
        self.volt_plot.addLegend()
        self.volt1_curve = self.volt_plot.plot(pen='b', name='Volt 1')
        self.volt2_curve = self.volt_plot.plot(pen='c', name='Volt 2')
        self.volt3_curve = self.volt_plot.plot(pen='m', name='Volt 3')
        self.volt4_curve = self.volt_plot.plot(pen='w', name='Volt 4')
        self.volt_plot.setLabel('left', 'Voltage (V)')
        self.volt_plot.setLabel('bottom', 'Time (s)')
        ...
        right_layout.addWidget(self.live_values_label)
        right_layout.addWidget(self.temp_plot)
        right_layout.addWidget(self.volt_plot)
            ...
            if line.startswith("can1"):
                ...
                if len(parts) > 2 and parts[1].lower() == "206":
                    try:
                        ...
                        self.temp1_curve.setData(self.time_history, self.temp1_history)
                        self.temp2_curve.setData(self.time_history, self.temp2_history)
                        self.temp3_curve.setData(self.time_history, self.temp3_history)

                        self.volt1_curve.setData(self.time_history, self.volt1_history)
                        self.volt2_curve.setData(self.time_history, self.volt2_history)
                        self.volt3_curve.setData(self.time_history, self.volt3_history)
                        self.volt4_curve.setData(self.time_history, self.volt4_history)
